package io.saul.citizenshiptracker.service.notification;
...
public class GMailNotificationService {
    ...
    @Autowired
    public GMailNotificationService(@Value("${gmail.enabled:false}") boolean enabled, @Value("${gmail.sender:nop}") String sender) throws IOException, GeneralSecurityException {
    ...
    public void send(String to, String subject, String bodyText) throws MessagingException, IOException {
        if(!enabled)return;
        sendMessage(gmailService, "me", createEmail(to, sender, subject, bodyText));
    }
}
